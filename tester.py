...
class LeNet300100(nn.Module):
    def __init__(self, in_features, hidden1_features, hidden2_features, out_features):
        ...
        # Define explicit weights and biases for each layer
        self.weight1 = nn.Parameter(torch.rand(hidden1_features, in_features))
        self.bias1 = nn.Parameter(torch.rand(hidden1_features))
        self.weight2 = nn.Parameter(torch.rand(hidden2_features, hidden1_features))
        self.bias2 = nn.Parameter(torch.rand(hidden2_features))
        self.weight3 = nn.Parameter(torch.rand(out_features, hidden2_features))
        self.bias3 = nn.Parameter(torch.rand(out_features))
        self.relu = nn.ReLU()

    def forward(self, x):
        # Custom linear layers with explicit weights and biases
        x = CustomLinearFunction.apply(x, self.weight1, self.bias1)
        x = self.relu(x)
        x = CustomLinearFunction.apply(x, self.weight2, self.bias2)
        x = self.relu(x)
        x = CustomLinearFunction.apply(x, self.weight3, self.bias3)
        return x
